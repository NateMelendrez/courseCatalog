...
const fetchCourses = asyncHandler(async (req: Request, res: Response) => {
  const { data: { data : course } } = await axios.get(API_URL, { headers: { Authorization: `Bearer ${API_KEY}` } });
  ...
  let filteredData: Course[] = course.map( (item: Item) => {
    ...
    attributes.forEach(attr => {
      if (item.hasOwnProperty(attr)){
        ...
        if (attr === "customFields") {
          const filteredCustomFields = item.customFields.filter((field: CustomField) =>
            customFieldIds.includes(field.field_id)
          );
  ...
  const { data: user } = await axios.get(`https://api.tovuti.io/api/v1/user/${userId}`, { headers: { Authorization: `Bearer ${API_KEY}` } });
  ...
  const userGroupIds = userGroup.filter((group: any) => user.userGroupIds.includes(group.id)).map(group => group.title);
